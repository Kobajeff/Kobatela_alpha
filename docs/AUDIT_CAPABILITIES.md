...
| POST | /spend/allow | `spend.allow_usage` | API key | n/a | `AllowedUsageCreate` | Statut dict | 200/201【F:app/routers/spend.py†L35-L37】 |
...
| POST | /spend/allowed | `spend.add_allowed_payee` | API key | n/a | `AddPayeeIn` | Dict payee | 201【F:app/routers/spend.py†L49-L74】 |
| POST | /spend | `spend.spend` | API key | n/a | `SpendIn` + `Idempotency-Key` | Dict paiement | 200【F:app/routers/spend.py†L77-L105】 |
| POST | /proofs | `proofs.submit_proof` | API key | n/a | `ProofCreate` | `ProofRead` | 201【F:app/routers/proofs.py†L1-L18】 |
| POST | /proofs/{id}/decision | `proofs.decide_proof` | API key | n/a | `ProofDecision` | `ProofRead` | 200/400【F:app/routers/proofs.py†L21-L33】 |
| POST | /payments/execute/{payment_id} | `payments.execute_payment` | API key | n/a | Path ID | `PaymentRead` | 200/404/409【F:app/routers/payments.py†L1-L17】 |
| POST | /psp/webhook | `psp.psp_webhook` | Aucun | n/a | Corps JSON + en-têtes PSP | Ack dict | 200/401/503【F:app/routers/psp.py†L1-L54】 |

## D. Data model & states
| Entity | Key fields | Constraints / Indexes | Notes |
| --- | --- | --- | --- |
| User | `username`, `email`, `is_active` | Unicité username/email | Identités de base liées aux transferts.【F:app/models/user.py†L1-L35】 |
| Alert | `type`, `message`, `actor_user_id` | Index sur type + `created_at` | Journal opérationnel.【F:app/models/alert.py†L1-L17】 |
| CertifiedAccount | `user_id`, `level` | Unicité `user_id`, enum `CertificationLevel` | Certification marchands/utilisateurs.【F:app/models/certified.py†L1-L28】 |
| EscrowAgreement | Parties, `amount_total`, `status`, `deadline_at` | Check >=0, index statut/deadline | Pilote la logique de preuves et dépôts.【F:app/models/escrow.py†L1-L68】 |
...
| EscrowEvent | `escrow_id`, `kind`, `data_json` | Index sur escrow/idempotency_key | Chronologie d'événements.【F:app/models/escrow.py†L58-L69】 |
| Milestone | `escrow_id`, `idx`, `amount`, géofence | Contraintes unicité + géofence | Support des preuves géolocalisées.【F:app/models/milestone.py†L1-L47】 |
| Proof | `escrow_id`, `milestone_id`, `sha256`, `status` | Unicité hash, index multiples | Stockage des pièces justificatives.【F:app/models/proof.py†L1-L24】 |
| Payment | `escrow_id`, `amount`, `psp_ref`, `status` | Check >0, unicité ref/idempotency | Suit le paiement externe.【F:app/models/payment.py†L1-L38】 |
| Transaction | `sender_id`, `receiver_id`, `amount`, `idempotency_key` | Check >0, idempotency unique | Transferts restreints.【F:app/models/transaction.py†L1-L37】 |
| AllowedRecipient | `owner_id`, `recipient_id` | Unicité paire | Allowlist restreinte.【F:app/models/allowlist.py†L1-L15】 |
...
| SpendCategory | `code`, `label` | Code unique | Classification politique.【F:app/models/spend.py†L1-L21】 |
...
| AllowedUsage | `owner_id`, `merchant_id/category_id` | Check exclusif + unicités | Règles de base hors mandat.【F:app/models/spend.py†L37-L53】 |
| Purchase | `sender_id`, `merchant_id`, `amount`, `status`, `idempotency_key` | Check >0, index statut, idempotency unique | Dépenses conditionnelles, maintenant en Decimal.【F:app/models/spend.py†L64-L83】 |
| UsageMandate | `sender_id`, `beneficiary_id`, `total_amount`, `status` | Check >=0, index multiples, enum statut | Mandat liant diaspora et bénéficiaire avec plafonds et date d'expiration.【F:app/models/usage_mandate.py†L22-L45】 |
| PSPWebhookEvent | `event_id`, `psp_ref`, `kind` | Unicité event, index | Idempotence webhook.【F:app/models/psp_webhook.py†L1-L21】 |
| AuditLog | `actor`, `action`, `entity`, `entity_id` | Timestamp auto, pas d'index secondaire | Piste d'audit financière.【F:app/models/audit.py†L1-L20】 |

**State machines**
- `EscrowStatus` : DRAFT → FUNDED → RELEASABLE → RELEASED / REFUNDED / CANCELLED, piloté par le service avec audits systématiques.【F:app/models/escrow.py†L12-L20】【F:app/services/escrow.py†L97-L213】
- `UsageMandateStatus` : ACTIVE → CONSUMED/EXPIRED, mis à jour lors des achats ou du nettoyage.【F:app/models/usage_mandate.py†L14-L45】【F:app/services/spend.py†L254-L258】【F:app/services/mandates.py†L84-L102】
- `PurchaseStatus` : COMPLETED par défaut, autres statuts réservés pour extension future.【F:app/models/spend.py†L56-L83】
- `PaymentStatus` : PENDING → SENT → SETTLED/ERROR/REFUNDED suivant les webhooks.【F:app/models/payment.py†L1-L38】【F:app/services/psp_webhooks.py†L60-L96】
- `MilestoneStatus` : WAITING → PENDING_REVIEW → APPROVED/REJECTED → PAYING → PAID selon les preuves.【F:app/models/milestone.py†L11-L31】【F:app/services/proofs.py†L172-L318】

## E. Stability results
- `alembic upgrade head` rejoue l'intégralité des trois révisions (init, Decimal, mandats) sans erreur.【672220†L1-L5】
- `alembic current` confirme la tête `5b91fcb4d6af`; `alembic heads` ne montre aucune branche parallèle.【121be4†L1-L4】【9ae31f†L1-L3】
- `pytest -q` : 29 tests passés, 0 échec/skip, en 1.03s.【855b8e†L1-L2】
- Aucune règle lint configurée; revue manuelle effectuée. Les helpers d'idempotence sont partagés entre dépenses, paiements, transactions et usage payees pour absorber les retries.【F:app/services/idempotency.py†L1-L51】【F:app/services/spend.py†L117-L286】【F:app/services/usage.py†L23-L235】

## F. Security & integrity
- **AuthN/AuthZ** : dépendance unique `require_api_key`, pas de rôles ni scopes, la fuite de clé donne accès complet à l'API.【F:app/security.py†L1-L21】【F:app/routers/__init__.py†L10-L20】
- **Mandats** : validations utilisateurs/merchants/catégories et date future mais absence d'audit et de filtre expéditeur lors de l'usage, créant un trou critique.【F:app/services/mandates.py†L21-L102】【F:app/services/spend.py†L135-L258】
- **PSP** : secret obligatoire, vérification HMAC/timestamp et audit des mises à jour de paiement.【F:app/routers/psp.py†L1-L54】【F:app/services/psp_webhooks.py†L1-L96】
- **Validation entrées** : Pydantic contrôle montants positifs, devises et structures sur escrows/spend/transactions.【F:app/schemas/escrow.py†L1-L57】【F:app/schemas/spend.py†L1-L86】【F:app/schemas/transaction.py†L1-L44】
- **Preuves fichiers** : contrôles EXIF, GPS et règles métiers avant de changer d'état milestone.【F:app/services/proofs.py†L45-L318】【F:app/services/rules.py†L15-L107】
- **Audit & logs** : logging JSON centralisé, audits sur escrows/paiements/achats mais pas sur mandats.【F:app/core/logging.py†L1-L34】【F:app/services/escrow.py†L1-L213】【F:app/services/spend.py†L243-L286】【F:app/services/mandates.py†L45-L102】

## G. Observability & ops
- Configuration logging via `setup_logging` exécutée dans le lifespan ; démarrage échoue si le secret PSP manque, ce qui sécurise la configuration.【F:app/main.py†L1-L61】【F:app/core/logging.py†L1-L34】
- Middleware CORS permissif (`*`) adapté au prototypage mais à restreindre en prod.【F:app/main.py†L31-L40】
- Gestion DB centralisée : engine/sessionmaker paresseux, PRAGMA SQLite forcé, dépendance FastAPI unique.【F:app/db.py†L1-L96】
- Migrations : trois révisions ordonnées (init, Decimal, mandats) avec contraintes et index alignés sur les modèles.【F:alembic/versions/baa4932d9a29_0001_init_schema.py†L1-L199】【F:alembic/versions/2c2680073b35_use_decimal_for_purchases_amount.py†L17-L58】【F:alembic/versions/5b91fcb4d6af_add_usage_mandates_table.py†L18-L87】
- Absence de tâches planifiées : nettoyage des mandats dépend d'une invocation API ; aucune intégration Cron/worker n'est fournie.【F:app/routers/mandates.py†L22-L27】
